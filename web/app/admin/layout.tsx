...
export default function AdminLayout({
  ...
  const isEditRoute = /\/admin\/companies\/[^/]+\/edit$/.test(pathname);
  const isDashboard = pathname === "/admin";

  let title = "概览";
  if (pathname === "/admin") title = "概览";
  else if (pathname === "/admin/companies") title = "公司管理";
  else if (pathname === "/admin/reviews") title = "口碑审核";
  else if (isEditRoute) title = "编辑公司";

  const navItems = [
    { href: "/admin", label: "概览", icon: LayoutDashboard },
    { href: "/admin/companies", label: "公司管理", icon: Building2 },
    { href: "/admin/reviews", label: "口碑管理", icon: MessageSquareText },
  ];
